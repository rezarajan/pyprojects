...
# WAL record format:
# [magic (4B)] [key_len (8B)] [key bytes] [value_len (8B)] [value bytes]
# [ts (8B)] [op (1B)] [crc32 (4B)]
        ...
        with open(self.path, "rb") as f:
            while True:
                ...
                if len(magic_bytes) < 4:
                    logger.warning("Partial record at EOF, skipping")
                    break
                ...
                if len(key_len_bytes) < 8:
                    logger.warning("Partial key_len at EOF, skipping")
                    break
                ...
                key = f.read(key_len)
                if len(key) < key_len:
                    logger.warning("Partial key at EOF, skipping")
                    break

                # Read value length
                value_len_bytes = f.read(8)
                if len(value_len_bytes) < 8:
                    logger.warning("Partial value_len at EOF, skipping")
                    break
                ...
                if len(value_bytes) < value_len:
                    logger.warning("Partial value at EOF, skipping")
                    break

                # Read timestamp
                ts_bytes = f.read(8)
                if len(ts_bytes) < 8:
                    logger.warning("Partial timestamp at EOF, skipping")
                    break
                ...
                op_bytes = f.read(1)
                if len(op_bytes) < 1:
                    logger.warning("Partial op_code at EOF, skipping")
                    break
                ...
                crc_bytes = f.read(4)
                if len(crc_bytes) < 4:
                    logger.warning("Partial CRC at EOF, skipping")
                    break
